  ...
  return (
    <div>
      <div className={classes["index-container"]}>
        <h1 className={classes["index-h1"]}>Dow Jones 30</h1>
        <AdvancedChart chart={props.chart} />
      </div>
      <IndexList data={props.list} title="Dow Jones 30 Constituents" />
    </div>
  );
};
...
export async function getServerSideProps(context: any) {
  const fetchHistoricalPrice = await fetch(
    `https://api.twelvedata.com/time_series?symbol=DJI&interval=1day&outputsize=5000&apikey=a24970c9566c49739e8009cdb3a639f0`
  );

  const historicalData = await fetchHistoricalPrice.json();
  const indexList = await fetch(
    `https://financialmodelingprep.com/api/v3/dowjones_constituent?apikey=b2643679ff15b6f250513b29b37b0375`
  );
